...
export const CartProvider = ({ children }) => {
  ...
  const removeFromCart = (itemId) => {
    setSelectedCards(selectedCards.filter((item) => item.id !== itemId));
  };
  const UpdatedQuantity = (quantity, action) => {
    let newQuantity = quantity;

    if (action === "decrease") {
      newQuantity -= 1;
      console.log("entropu aq", newQuantity);
    }
    if (action === "increase") {
      newQuantity = newQuantity + 1;
    }
    console.log("miha nvoaquantidade", newQuantity);
  };
